...
const BuySuccess: React.FC<BuySuccessProps> = ({ id, hide, reload, title, bType, isShowing }) => {
  const displayTitle = title || bType || "Bhero";
  const oke = () => {
    if (reload) reload();
    hide();
  };
    ...
    <ContentModal open={isShowing} footer={false} onCancel={hide}>
      <div className="head-modal">
        ...
        <div className="agency">
          {displayTitle} <span>#{id}</span>
        </div>
      </div>

      <div className="content">
        <img src="/icons/buy_success.png" alt="" />
      </div>
      <div className="des-bottom agency">Buy success</div>
